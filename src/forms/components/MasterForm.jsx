...
export default function MasterForm({ config = {}, total: initialTotal = 0, onCreate, onDelete, tableKey, tableName, onRowDoubleClick, pickerMode = false, onPickerSelect }) {
    ...
    const infiniteScrollEnabled = config.infiniteScroll === true;
    const hidePagination = config.hidePagination === true || infiniteScrollEnabled;
    const infiniteScrollHasMore = rows.length < total;

    // Reset accumulated rows when search changes so infinite scroll restarts from page 1
    useEffect(() => {
        if (infiniteScrollEnabled) {
            setRows([]);
            setTotal(0);
        }
    }, [quickSearch, infiniteScrollEnabled]);
    ...
    const handleFetchData = useCallback(async (queryString) => {
        ...
        // Guard: skip fetch if we already have all records (only for subsequent pages)
        if (!isFirstPage && rows.length > 0 && total > 0 && rows.length >= total) return;

        const mergeWithDedup = (prev, incoming) => {
            if (isFirstPage) return incoming;
            const existingIds = new Set(prev.map((row) => row?.id));
            const deduped = incoming.filter((row) => !existingIds.has(row?.id));
            return [...prev, ...deduped];
        };
        ...
        try {
            ...
            if (Array.isArray(dataNode)) {
                setRows((prev) => mergeWithDedup(prev, dataNode));
                setTotal(dataNode.length);
            } else if (dataNode && Array.isArray(dataNode.data)) {
                const newData = dataNode.data || [];
                const newTotal = dataNode.total || newData.length || 0;
                setRows((prev) => mergeWithDedup(prev, newData));
                setTotal(newTotal);
            } else {
                if (isFirstPage) {
                    setRows([]);
    ...
    }, [config.endpoint, config.readFrom, showLoader, hideLoader, rows.length, total]);
